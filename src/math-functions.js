...
export function sum(a, b) {
    let sumArray = [];
    let totalSum = a + b; 
    sumArray.push(totalSum, `The sum of ${a} and ${b} is ${totalSum}.`);
    return sumArray;
}
...
//Katie & Yosef helped with this.
export function sumArrayWithThreeNumbers(sumArr) {
    let totalSum = (sum((sum(sumArr[0], sumArr[1])[0]), sumArr[2]))[0];
    let firstItem = `${sumArr} was passed in as an array of numbers, and ${totalSum} is their sum.`;
    let mathArray = [];
    mathArray.push(totalSum, firstItem);
    return mathArray;
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    let totalProduct = (multiply((multiply(multArr[0], multArr[1])[0]), multArr[2]))[0];
    let firstItem = `$ The numbers ${multArr} have a product of ${totalProduct}.`;
    let mathArray = [];
    mathArray.push(totalProduct, firstItem);
    return mathArray;
}
